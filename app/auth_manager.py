...
class AuthManager:
    ...
    def start_google_drive_auth(self, remote_name: str = "gdrive") -> Dict[str, str]:
        """Start Google Drive OAuth authentication process with web redirect"""
        try:
            state = secrets.token_urlsafe(32)
            self.config_dir.mkdir(parents=True, exist_ok=True)
            
            # Create rclone config with drive remote (without token initially)
            config_template = f"""[{remote_name}]
type = drive
scope = drive
"""
            ...
            elif f"[{remote_name}]" not in self.config_path.read_text():
                existing = self.config_path.read_text()
                self.config_path.write_text(existing + "\n" + config_template)
            ...
            auth_url = (
                ...
                "redirect_uri=http://localhost:8000/api/auth/google-drive/callback&"
                "response_type=code&"
